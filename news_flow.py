...
def title_key(t):
    """标题归一化指纹: 用于跨来源/跨链接识别"同一条新闻", 避免重复推送。
    去掉大小写、标点、空格, 只保留字母数字和中文, 取前 80 字符。"""
    t = (t or "").lower()
...
def main():
    ...
    # 双重去重: 链接(uid)没见过, 且标题指纹也没见过(跨来源/跨链接的同一条新闻只算一次);
    # 同一批里出现的同标题也只保留第一条。
    all_new = []
    batch_keys = set()
    for it in entries:
        ...
        if uid in seen_set or (tkey and tkey in seen_set) or (tkey and tkey in batch_keys):
            continue
        if tkey:
            batch_keys.add(tkey)
        it["tkey"] = tkey
